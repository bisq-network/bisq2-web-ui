...
@Getter
@Slf4j
public class WebApplicationService extends ApplicationService {
...
//                .thenCompose(result -> bisqEasyService.initialize())
    ...
    private void setState(State newState) {
        if (state.get().ordinal() >= newState.ordinal()) {
            throw new IllegalArgumentException(String.format("New state %s must have a higher ordinal as the current state %s", newState, state.get()));
        }
